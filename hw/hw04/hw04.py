def shuffle(s):
    ...
    def shuffle_two(a,b):
        if len(a) == 0:
            ...
            return [a[0], b[0]] + shuffle_two(a[1:],b[1:])
    
    s = list(s)

    s0, s1 = s[:int(len(s)/2)], s[int(len(s)/2):]

    return shuffle_two(s0,s1)
...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return True if mass(end(left(m)))*length(left(m))==mass(end(right(m)))*length(right(m)) else False
    else:
        return True if total_mass(end(left(m)))*length(left(m))==total_mass(end(right(m)))*length(right(m)) and balanced(end(left(m))) and balanced(end(right(m))) else False
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        maxes = [max_path_sum(br) for br in branches(t)]
        return max(maxes)+label(t)
