...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    opp_tenth= (opponent_score // 10)%10
    player_digit = player_score%10
    if opp_tenth == player_digit:
        return 1
    else:
        return 3*(abs(opp_tenth-player_digit))
...
def make_averaged(original_function, times_called=1000):
    ...
    def f(*args):
        ...
        while count <= times_called:
            output_sum, count = output_sum+ original_function(*args), count + 1

        return output_sum/times_called
    return f
    # END PROBLEM 8
