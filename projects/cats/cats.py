...
def accuracy(typed, source):
    ...
    for i in range(len(typed_words)):
        if len(source_words)<i+1:
            score = score
        elif typed_words[i] == source_words[i]:
            score += 1
    
    if len(typed_words)==0:
        return 0 if source_words else 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    diff_typed = lambda x: diff_function(typed_word, x, limit)

    if diff_typed(max(word_list, key=diff_typed)) > limit:
        return typed_word
    else:
        return min(word_list, key=diff_typed)
...
def furry_fixes(typed, source, limit):
    ...
    """
    """(Attempt 1)
    m, M = min(len(typed), len(source)), max(len(typed), len(source))

    typed, source = typed[:m], source[:m]

    if M-m > limit:
        return limit + 1
    
    def furry_help (typed, source, limit, diff):
        if diff > limit:
            return limit + 1
        if not typed and not source:
            ...
            if typed[0]!= source[0]:
                return furry_help(typed[1:], source[1:], limit, diff + 1)
            else:
                return furry_help(typed[1:], source[1:], limit, diff)
    
    return furry_help(typed, source, limit, M-m)
    """

    if not source or not typed:
        print('returning difference:', max (len(typed), len(source))-min (len(typed), len(source)))
        return max (len(typed), len(source))
    else:
        prev = furry_fixes(typed[1:], source[1:], limit)
        if prev >limit:
            return 1+limit
        if typed[0]!= source[0]:
            print('adding one')
...
def minimum_mewtations(typed, source, limit):
    ...
    >>> big_limit = 10
    >>> minimum_mewtations("cats", "scat", 10)       # cats -> scats -> scat
    2
    ...
    if not source or not typed:
        ...
        return max(len(typed), len(source))+10
    else:
        if typed[0] == source[0]:
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(times)):
        cur_list = tpp[i]
        for k in range(len(cur_list)-1):
            times[i].append(cur_list[k+1]-cur_list[k])
...
def fastest_words(words_and_times):
    ...
    for i in player_indices:
        ...
        for k in word_indices:
            word = words[k]
            wmin= min([get_time(times, p, k) for p in player_indices])
            if word not in wsf and get_time(times, i, k)==wmin:
                player.append(word)
        list.append(player)
